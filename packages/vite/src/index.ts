 ...
 * per scanned `collecting()` category. The SDK's `ScannedCollectionKeys`
 * interface is augmented by this file so `defineConfig` requires every
 * scanned category to have a matching entry in `data.purposes`. Commit the
 * file — that keeps the type-level constraint live in CI without needing to
 * run the Vite plugin first.
 */
async function writeAutoCollectedDts(
	targetDir: string,
	dataCollected: Record<string, string[]>,
): Promise<void> {
	const keys = Object.keys(dataCollected).sort();
	const body = keys.length ? keys.map((k) => `\t\t${JSON.stringify(k)}: true;`).join("\n") : "";
	const contents =
...
export function openPolicy(options: OpenPolicyOptions = {}): Plugin {
	...
	async function rescanAndRefresh(server: ViteDevServer): Promise<void> {
		...
		scanned = next;
		await writeAutoCollectedDts(resolvedConfigDir, scanned.dataCollected);
