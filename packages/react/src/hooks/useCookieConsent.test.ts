...
test("getSnapshotCached returns referentially stable value when cookie unchanged", () => {
	acceptAllForConfig(cookieConfig);
	const a = getSnapshotCached();
	const b = getSnapshotCached();
	expect(a).toBe(b); // same reference, not just deep equal
});

test("getSnapshotCached returns new reference after consent changes", () => {
	acceptAllForConfig(cookieConfig);
	const a = getSnapshotCached();
	rejectAllForConfig();
	const b = getSnapshotCached();
...
test("getSnapshotCached returns null when no consent is set", () => {
	expect(getSnapshotCached()).toBeNull();
});
