...
config lives in [`src/openpolicy.ts`](src/openpolicy.ts) — the only
...
generate `src/openpolicy.gen.ts`).
...
src/openpolicy.ts             ← defineConfig() — framework-neutral, fully commented
src/openpolicy.gen.ts         ← generated by openPolicy()'s source scan
