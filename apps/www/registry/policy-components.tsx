...
export function PolicyHeading({ node }: { node: HeadingNode }) {
	...
	if (level <= 1)
		return (
			<h1 className="text-xl font-semibold text-gray-900 mb-3">{node.value}</h1>
		);
	if (level === 2)
		return (
	...
	return (
		<h3 className="text-sm font-semibold text-gray-900 mb-2">{node.value}</h3>
	);
}
...
export function PolicyList({
	...
	return (
		<Tag className="text-sm text-gray-600 pl-5 mb-3 space-y-1 list-disc">
			{children}
		</Tag>
	);
}
