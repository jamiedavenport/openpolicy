...
import { FeatureGrid, Install, ProductHero } from "./openpolicy";
...
const INSTALL_SNIPPET = `pnpm add @opencookies/core @opencookies/react`;

const USE_SNIPPET = `import { useConsent } from "@opencookies/react";
...
export const Route = createFileRoute("/opencookies")({
	component: OpenCookies,
	head: (ctx) =>
		pageMeta(
			{
				title: "OpenCookies — a headless consent state machine | PolicyStack",
				description:
	...
	return (
		<>
			<ProductHero
				...
				primary={{ label: "pnpm add @opencookies/react", href: "#install", icon: "arrow-right" }}
				secondary={{
					label: "github",
