...
- [`@opencookies/vite`](/docs/opencookies/vite) — Vite plugin that runs the scanner during dev and CI (recommended; you usually don't call the scanner directly)
- [`@opencookies/cli`](/docs/opencookies/cli) — terminal entry point for one-off scans and config sync
- [`@opencookies/core`](/docs/opencookies/core) — runtime that actually enforces the consent decisions the scanner is checking for
