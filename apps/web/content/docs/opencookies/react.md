...
Already using OpenPolicy? Skip the hand-rolled categories array — `toOpenCookiesConfig(openpolicy)` from `@policystack/sdk/opencookies` produces this config from your `openpolicy.ts` and defaults `policyVersion` from `cookieVersion` so `triggers.policyVersionChanged` reprompts correct-by-default. See [Cookie banner](/docs/openpolicy/cookies/overview).
