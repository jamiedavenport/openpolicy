...
The CLI installs the right packages for your stack, scaffolds an `openpolicy.ts`, and prints a prompt tuned for coding agents — paste it into Claude Code and it'll fill in the company details, infer your jurisdictions, declare the third parties from your `package.json`, and add a `/privacy` route in seconds. `@openpolicy/react` also ships a Claude Code Skill (`render-policies`) that the agent picks up automatically once installed, so it knows the renderer API without you explaining it.
...
The SDK gives you the config types and helpers. `@openpolicy/react` renders the config straight into your component tree. The Vite plugin scans your codebase for things you should be declaring (more on this below).
